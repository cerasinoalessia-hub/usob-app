...
export default async function CalendarioPage() {
  const { data: partite } = await supabase
    .from("partite")
    .select("*, squadre(nome, indirizzo, colori)")
  ...
  function MatchRow({ p }: { p: any }) {
    const isHome = p.casa;
    const label = isHome ? `USOB vs ${p.squadre?.nome ?? p.squadra_avversaria}` : `${p.squadre?.nome ?? p.squadra_avversaria} vs USOB`;
      ...
      <Link href={`/calendario/${p.id}`} className="flex items-center justify-between py-3 border-b border-gray-100 last:border-0">
        <div>
          <p className="text-xs text-gray-400">
            {new Date(p.data).toLocaleDateString("it-IT", { weekday: "short", day: "2-digit", month: "short" })}
            {p.squadre?.indirizzo ? ` · ${p.squadre.indirizzo}` : ""}
          </p>
          <p className="text-sm font-medium text-gray-800">{label}</p>
        </div>
        {hasFinalScore ? (
          <span className="text-sm font-bold text-usob-blue">{p.gol_casa} – {p.gol_trasferta}</span>
        ) : (
