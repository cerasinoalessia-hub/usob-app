    ...
    <div>
        ...
        {(marcatori ?? []).length === 0 ? <p className="text-sm text-gray-400">Nessun gol registrato</p> :
          (marcatori ?? []).map((m: any, i: number) => (
            <div key={i} className="flex items-center gap-3 py-2.5 border-b border-gray-50 last:border-0">
              ...
              <div className="flex-1"><p className="text-sm font-medium">{m.cognome} {m.nome}</p><p className="text-xs text-gray-400">{m.ruolo}</p></div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-4">
        ...
        {(classifica ?? []).map((r: any, i: number) => (
          <div key={i} className={`flex items-center gap-2 py-2.5 border-b border-gray-50 last:border-0 rounded-lg px-1 ${r.usob ? "bg-[#fef9e7]" : ""}`}>
            ...
            <span className="text-sm font-bold text-[#1a3a6b]">{r.punti} pt</span>
          </div>
        ))}
      </div>
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-4">
        ...
        {ruoli.map((ruolo) => {
          ...
          return (
            <div key={ruolo} className="mb-4">
              <p className="text-xs font-bold text-[#1a3a6b] mb-2">{ruoloLabel[ruolo]}</p>
